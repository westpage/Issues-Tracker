...
function fetchIssueList() {
  ...
  if (!ourLocalStorage) {
    ...
    for (var i = 0; i < ourLocalStorage.length; i++) {
      ...
      issuesContainer.innerHTML +=
        ' <div class="w-full sm:max-w-[550px] md:max-w-[340px] lg:max-w-[420px] mt-6 bg-white shadow-sm  hover:shadow-lg rounded-md">' +
        ...
        ' <span class="bg-gray-50 shadow-inner text-green-500  font-semibold rounded-xl text-xs ml-2 px-2 py-1">' +
        status +
        "  </span>" +
        "</p>" +
        "</div>" +
        ...
        '<a href=""  onclick="setStatusClosed(\'' +id+ '\')" class="text-xs text-blue-500 font-semibold rounded-md shadow-sm hover:shadow-xl px-2 py-1">Close </a>' +
        '<a href=""  onclick="deleteStatus(\'' +id+ '\')" class="text-xs text-red-500 bg-gray-50 font-semibold rounded-md shadow-sm hover:shadow-xl px-2 py-1">Delete </a> ' 
        +"</div>";
